...
void usbInit_1608G(libusb_device_handle *udev, int version)
{
  ...
  */
  ...
  if (version == 1) {  // older version
    if (!(usbStatus_USB1608G(udev) & USB1608G_FPGA_CONFIGURED)) {
      usbFPGAConfig_USB1608G(udev);
      if (usbStatus_USB1608G(udev) & USB1608G_FPGA_CONFIG_MODE) {
...
void usbAInScanStart_USB1608G(libusb_device_handle *udev, usbDevice1608G *usb1608G)
{
  ...
  */
  ...
  if (usb1608G->frequency == 0.0) {
    ...
    AInScan.pacer_period = rint((USB1608G_BASE_CLOCK / usb1608G->frequency) - 1);
  }
...
{
  char value[USB1608G_PACKET_SIZE];
  int ret = -1;
  ...
  usb1608G->status = usbStatus_USB1608G(udev);
  if ((usb1608G->status & USB1608G_AIN_SCAN_OVERRUN)) {
    perror("Scan overrun.");
    usbAInScanStop_USB1608G(udev);
    usbAInScanClearFIFO_USB1608G(udev);
  }
...
void usbAInConfig_USB1608G(libusb_device_handle *udev, usbDevice1608G *usb1608G)
{
  /*
    ...
    mode:   USB1608G_SINGLE_ENDED  (Single-Ended)
            USB1608G_DIFFERENTIAL  (Differential)
            USB1608G_CALIBRATION   (Calibration mode)
            USB1608G_LAST_CHANNEL  (End of scan)               
  ...
  */
  ...
  for (i = 0; i < NCHAN_1608G; i++) {
    ...
    } else if ((usb1608G->list[i].mode & 0x3) == USB1608G_DIFFERENTIAL && usb1608G->list[i].channel >= 0 && usb1608G->list[i].channel < 8) {
      usb1608G->scan_list[i] =  (usb1608G->list[i].channel & 0x7);
    } else if ((usb1608G->list[i].mode & 0x3) == USB1608G_CALIBRATION) {
...
int usbAOutWrite_USB1608GX_2AO(libusb_device_handle *udev, uint16_t *data, int nBytes)
{
  ...
  if (usbStatus_USB1608G(udev) & USB1608G_AOUT_SCAN_UNDERRUN) {
  ...
  if ((ret = libusb_bulk_transfer(udev, LIBUSB_ENDPOINT_OUT|2, (unsigned char *) data, nBytes, &transferred, 400)) < 0) {
    if (usbStatus_USB1608G(udev) & USB1608G_AOUT_SCAN_RUNNING) {
...
void usbAOutScanStart_USB1608GX_2AO(libusb_device_handle *udev, uint32_t count, uint32_t retrig_count, double frequency, uint8_t options)
{
  ...
  */  
  ...
  AOutScan.pacer_period = (USB1608G_BASE_CLOCK / frequency) - 1;
...
void usbTimerPeriodR_USB1608G(libusb_device_handle *udev, float *period)
{
  ...
  *period = (value + 1)*1000./USB1608G_BASE_CLOCK;
}    
...
void usbTimerPeriodW_USB1608G(libusb_device_handle *udev, float period)
{
  ...
  value = rint(period*USB1608G_BASE_CLOCK/1000. - 1);
  wValue = value & 0xffff;
...
void usbTimerPulseWidthR_USB1608G(libusb_device_handle *udev, float *pulseWidth)
{
  ...
  *pulseWidth = (pulse_width + 1)*1000./USB1608G_BASE_CLOCK;
}
...
void usbTimerPulseWidthW_USB1608G(libusb_device_handle *udev, float pulseWidth)
{
  ...
  pulse_width = rint(pulseWidth*USB1608G_BASE_CLOCK/1000. - 1);
...
void usbTimerDelayR_USB1608G(libusb_device_handle *udev, float *delay)
{
  ...
  *delay = value*1000./USB1608G_BASE_CLOCK;
}
...
void usbTimerDelayW_USB1608G(libusb_device_handle *udev, float delay)
{
  ...
  value = rint(delay*USB1608G_BASE_CLOCK/1000.);
  wValue = value & 0xffff;
  wIndex = (value >> 16) & 0xffff;
